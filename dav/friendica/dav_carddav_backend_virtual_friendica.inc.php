...
class Sabre_CardDAV_Backend_Friendica extends Sabre_CardDAV_Backend_Virtual
{
     ...
     */
    protected static function createCache_internal($addressbookId, $force = false)
    {
        ...
        if (count($addressbook) != 1 || $addressbook[0]['namespace'] != CARDDAV_NAMESPACE_PRIVATE) {
     ...
     * @throws Sabre_DAV_Exception_NotFound
     */
    protected static function createCardCache($addressbookId, $contactId, $force = false)
    {
        $addressbook = q('SELECT * FROM %s%saddressbooks WHERE `id` = %d', CALDAV_SQL_DB, CALDAV_SQL_PREFIX, intval($addressbookId));
        if (count($addressbook) != 1 || $addressbook[0]['namespace'] != CARDDAV_NAMESPACE_PRIVATE) {
            throw new Sabre_DAV_Exception_NotFound();
