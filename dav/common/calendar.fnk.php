...
class vcard_source_data_homepage
{
    ...
    public function __construct($type, $homepage)
    {
...
class vcard_source_data_telephone
{
    ...
    public function __construct($type, $telephone)
    {
...
class vcard_source_data_socialnetwork
{
    ...
    public function __construct($type, $nick, $url)
    {
        $this->nick = $nick;
        $this->type = $type;
        $this->url = $url;
...
{
    public function __construct($name_first, $name_middle, $name_last)
    {
...
function vcard_source_compile($vcardsource)
{
    ...
    for ($i = 0; $i < count($vcardsource->homepages); ++$i) {
        if ($i == 0) {
            $str .= 'URL;type='.$vcardsource->homepages[0]->type.':'.$vcardsource->homepages[0]->homepage."\r\n";
        } else {
            $c = ++$item_count;
    ...
    if (isset($vcardsource->socialnetworks) && is_array($vcardsource->socialnetworks)) {
        foreach ($vcardsource->socialnetworks as $netw) {
            switch ($netw->type) {
        case 'dfrn':
...
function dav_get_current_user_calendars(&$server, $with_privilege = '')
{
    ...
    foreach ($children as $child) {
        if (is_a($child, 'Sabre_CalDAV_Calendar') || is_subclass_of($child, 'Sabre_CalDAV_Calendar')) {
            if ($with_privilege != '') {
                $caluri = $calendar_path.$child->getName();
                if ($aclplugin->checkPrivileges($caluri, $with_privilege, Sabre_DAVACL_Plugin::R_PARENT, false)) {
                    $calendars[] = $child;
                }
            } else {
                $calendars[] = $child;
