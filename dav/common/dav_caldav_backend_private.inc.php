...
class Sabre_CalDAV_Backend_Private extends Sabre_CalDAV_Backend_Common
{
    ...
    {
        if ($calendar['namespace'] == CALDAV_NAMESPACE_PRIVATE && $user == $calendar['namespace_id']) {
    ...
    public function getCalendarsForUser($principalUri)
    {
        ...
        foreach ($cals as $cal) {
            if (!isset($cal['uri'])) {
                throw new DAVVersionMismatchException();
            }
    ...
    public function getCalendarObject($calendarId, $objectUri)
    {
        ...
        if (count($o) > 0) {
            ...
            throw new Sabre_DAV_Exception_NotFound($calendarId.' / '.$objectUri);
        }
    }
