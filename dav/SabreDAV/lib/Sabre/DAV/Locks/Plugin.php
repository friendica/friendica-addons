...
class Sabre_DAV_Locks_Plugin extends Sabre_DAV_ServerPlugin
{
    ...
    public function unknownMethod($method, $uri)
    {
        switch ($method) {
            ...
            case 'LOCK': $this->httpLock($uri);

return false;
            case 'UNLOCK': $this->httpUnlock($uri);
...
return false;
    ...
    protected function httpUnlock($uri)
    {
        ...
        if (!$lockToken) {
            throw new Sabre_DAV_Exception_BadRequest('No lock token was supplied');
        }
    ...
    public function getTimeoutHeader()
    {
        ...
        if ($header) {
            ...
            } elseif (strtolower($header) == 'infinite') {
                $header = Sabre_DAV_Locks_LockInfo::TIMEOUT_INFINITE;
            } else {
                throw new Sabre_DAV_Exception_BadRequest('Invalid HTTP timeout header');
            }
        } else {
            $header = 0;
        }
