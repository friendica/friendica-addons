    ...
    if (x($_REQUEST, 'realtime_cb') && x($_REQUEST, 'realtime_cb')) {
        ...
        foreach ($js->entry as $entry) {
            ...
            foreach ($entry->changed_fields as $field) {
                ...
                if (in_array($fbuser, $affected_users[$field])) {
                    continue;
                }
...
function fb_get_friends_sync_full($uid, $access_token, $persons)
{
    ...
    for ($i = 0; $i < $nums; ++$i) {
        $batch_request = array();
        for ($j = $i * 50; $j < ($i + 1) * 50 && $j < count($persons); ++$j) {
            $batch_request[] = array('method' => 'GET', 'relative_url' => $persons[$j]->id);
        }
        ...
        if ($s) {
            ...
            if (count($results)) {
                foreach ($results as $contact) {
                    if ($contact->code != 200) {
                        logger('fb_get_friends: not found: '.print_r($contact, true), LOGGER_DEBUG);
                    } else {
                        fb_get_friends_sync_parsecontact($uid, json_decode($contact->body));
                    }
                }
            }
        }
...
function facebook_plugin_admin(&$a, &$o)
{
    ...
    if ($appid && $appsecret) {
        ...
        if ($subs === null) {
            $o .= t('Error: the given API Key seems to be incorrect (the application access token could not be retrieved).').'<br>';
        } elseif (is_array($subs)) {
            $o .= t('The given API Key seems to work correctly.').'<br>';
            $working_connection = true;
        } else {
            $o .= t('The correctness of the API Key could not be detected. Something strange\'s going on.').'<br>';
        }
    }
...
function facebook_plugin_admin_post(&$a)
{
    ...
    if (x($_REQUEST, 'fb_save_keys')) {
        ...
        if ($poll_interval >= FACEBOOK_MIN_POLL_INTERVAL) {
            set_config('facebook', 'poll_interval', $poll_interval);
        }
    ...
    if (strstr($b['postopts'], 'facebook') || ($b['private']) || ($reply)) {
        ...
        if ($appid && $secret) {
            ...
            if ($fb_token && ($toplevel || $b['private'] || $reply)) {
                ...
                } elseif (($link != '') or ($image != '') or ($b['title'] == '') or (strlen($msg) < 500)) {
    ...
    foreach ($j->data as $entry) {
        ...
        if (!$orig_post) {
            ...
            if (isset($entry->place)) {
                ...
                } elseif ($entry->place->location->latitude and $entry->place->location->longitude) {
    ...
    if ($j) {
        ...
        if (isset($x->error)) {
            ...
            if ($x->error->type == 'OAuthException' && $x->error->code == 190) {
                del_config('facebook', 'app_access_token');
                if ($second_try === false) {
                    facebook_subscription_add_users(true);
                }
            }
        } else {
            logger('facebook_subscription_add_users: sucessful');
            if (facebook_check_realtime_active()) {
                set_config('facebook', 'realtime_active', 1);
            }
        }
    }
}
...
function facebook_check_realtime_active()
{
    ...
    if (is_array($ret)) {
        foreach ($ret as $re) {
            if (is_object($re) && $re->object == 'user') {
                return true;
            }
        }
    }
