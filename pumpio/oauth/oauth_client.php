...
class oauth_client_class
{
	...
	*/
	...
	public function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		if (isset($oauth)) {
			...
			if (count($files)) {
				...
				$value_parameters = ($type !== 'application/x-www-form-urlencoded' ? [] : $parameters);
			}
	...
	public function ProcessToken($code, $refresh)
	{
		if ($refresh) {
			$values = [
		...
		if (!$this->SendAPIRequest($url, 'POST', $values, null, ['Resource' => 'OAuth ' . ($refresh ? 'refresh' : 'access') . ' token', 'ConvertObjects' => true], $response)) {
			return false;
		}
		...
		switch (intval($this->oauth_version)) {
			case 1:
				...
				&& isset($access_token['value'])) {
					...
					|| $expired) {
						...
						&& !isset($verifier))) {
							...
							$options                   = ['Resource' => 'OAuth access token'];
							...
							if (!$this->SendAPIRequest($url, $method, [], $oauth, $options, $response)) {
								return false;
							}
							if (strlen($this->access_token_error)) {
								$this->authorization_error = $this->access_token_error;
				...
				if (!isset($access_token['authorized'])) {
					...
					$options = ['Resource' => 'OAuth request token'];
					$method  = strtoupper($this->token_request_method);
					switch ($method) {
						case 'GET':
							break;
